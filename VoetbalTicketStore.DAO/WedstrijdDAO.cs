...
namespace VoetbalTicketStore.DAO
{
    public class WedstrijdDAO : IWedstrijdDAO
    {
        ...
        public IEnumerable<Wedstrijd> GetAanTeRadenWedstrijdenVoorClub(int? clubId, int aantal)
        {
            using (var db = new VoetbalstoreEntities())
            {
                return db.Wedstrijds.Where(w => w.Club1id == clubId && w.DatumEnTijd > DateTime.Now).OrderBy(w => w.DatumEnTijd).Take(aantal).Include(w => w.Club).Include(w => w.Club1).ToList();
            }
        }
